...
public class PGIndexDatabase extends PGObjectStore implements IndexDatabase {
	...
	@Inject
	public PGIndexDatabase(final ConfigDatabase configdb, final Hints hints) throws URISyntaxException {
	...
	public PGIndexDatabase(final ConfigDatabase configdb, final Environment config, final boolean readOnly) {
	...
	public IndexInfo updateIndexInfo(String treeName, String attributeName, IndexType strategy,
		...
		IndexInfo index = new IndexInfo(treeName, attributeName, strategy, metadata);
		final String deleteSql = format("DELETE FROM %s WHERE repository = ? AND treeName = ? AND attributeName = ?",
				config.getTables().index());
	...
	public Optional<IndexInfo> getIndexInfo(String treeName, String attributeName) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			try (PreparedStatement ps = cx.prepareStatement(log(sql, LOG, repositoryId, treeName, attributeName))) {
				ps.setInt(1, repositoryId);
				ps.setString(2, treeName);
				ps.setString(3, attributeName);
				try (ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						...
						if (metadataBytes != null) {
							try (ByteArrayInputStream metadataStream = new ByteArrayInputStream(metadataBytes)) {
	...
	public List<IndexInfo> getIndexInfos(String treeName) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			try (PreparedStatement ps = cx.prepareStatement(log(sql, LOG, repositoryId, treeName))) {
				ps.setInt(1, repositoryId);
				ps.setString(2, treeName);
				try (ResultSet rs = ps.executeQuery()) {
					while (rs.next()) {
	...
	public List<IndexInfo> getIndexInfos() {
		final String sql = format("SELECT treeName, attributeName, strategy, metadata FROM %s WHERE repository = ?",
				config.getTables().index());
	...
	public boolean dropIndex(IndexInfo index) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			cx.setAutoCommit(false);
			try (PreparedStatement ps = cx.prepareStatement(
					log(deleteSql, LOG, repositoryId, index.getTreeName(), index.getAttributeName()))) {
				ps.setInt(1, repositoryId);
	...
	public void clearIndex(IndexInfo index) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			cx.setAutoCommit(false);
			try (PreparedStatement ps = cx.prepareStatement(log(deleteSql, LOG, repositoryId, pgIndexId))) {
				ps.setInt(1, repositoryId);
	...
	public void addIndexedTree(IndexInfo index, ObjectId originalTree, ObjectId indexedTree) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			...
			try {
				...
				try (PreparedStatement ps = cx
						.prepareStatement(log(insertSql, LOG, repositoryId, pgIndexId, pgTreeId, pgIndexedTreeId))) {
					ps.setInt(1, repositoryId);
	...
	public Optional<ObjectId> resolveIndexedTree(IndexInfo index, ObjectId treeId) {
		...
		try (Connection cx = PGStorage.newConnection(dataSource)) {
			try (PreparedStatement ps = cx.prepareStatement(log(sql, LOG, repositoryId, pgIndexId, pgTreeId))) {
				ps.setInt(1, repositoryId);
				ps.setInt(2, pgIndexId.hash1());
				pgIndexId.setArgs(ps, 3);
				ps.setInt(6, pgTreeId.hash1());
