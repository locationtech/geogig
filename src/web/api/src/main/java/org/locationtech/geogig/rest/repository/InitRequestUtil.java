...
public class InitRequestUtil {
    ...
    /**
     * Updates the provided Hints instance with a GeoGig Repository URL based on the supplied
     * parameter map. This method builds the Repository location URI form the supplied parameter map
     * and sets it on the provided Hints instance so that the RepositoryResolver can correctly build
     * the repository at the requested location.
     *
    ...
    private void updateHintsWithParams(Hints hints, Map<String, String> params)
        ...
        if (parentDir != null) {
            ...
            final RepositoryResolver resolver = RepositoryFinder.INSTANCE.lookup(parentUri);
            ...
            pathBuilder.append(SLASH).append(dbName).append(SLASH).append(dbSchema).append(SLASH)
                    .append(repoName);
            // now build the query part, i.e. user=username&password=password
            // need to URLEncode these to make sure things are encoded that need to be, and things
            // don't get double
            // encoded
            ...
            queryBuilder.append(USER).append('=').append(URLEncoder.encode(dbUser, UTF8))
                    ...
                    .append(URLEncoder.encode(dbPassword, UTF8));
            ...
            final URI repoUri = new URI(SCHEME, null, dbHost, port, pathBuilder.toString(),
