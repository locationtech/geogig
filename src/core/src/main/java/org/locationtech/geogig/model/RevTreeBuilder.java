...
public class RevTreeBuilder {
    ...
    public static RevTreeImpl createLeafTree(long size, Collection<Node> features,
        ...
        return RevTreeBuilder.createLeafTree(id, size, featuresList, treesList);
    }
    ...
    private RevTreeImpl createNodeTree(long size, int numTrees, TreeMap<Integer, Bucket> buckets) {
        ...
        return RevTreeBuilder.createNodeTree(id, size, numTrees, innerTrees);
    }
    ...
    public static RevTreeImpl createLeafTree(ObjectId id, long size, ImmutableList<Node> features,
        ...
        return new RevTreeImpl.LeafTree(id, size, f, t);
    }
    ...
    public static RevTreeImpl createNodeTree(final ObjectId id, final long size,
        ...
        Preconditions
                .checkArgument(bucketTrees.size() <= CanonicalNodeNameOrder.maxBucketsForLevel(0));

        ImmutableSortedMap<Integer, Bucket> innerTrees = ImmutableSortedMap.copyOf(bucketTrees);

        return new RevTreeImpl.NodeTree(id, size, childTreeCount, innerTrees);
    }
    ...
    public static RevTreeImpl createLeafTree(ObjectId id, long size, Collection<Node> features,
        ...
        final CanonicalNodeOrder ordering = CanonicalNodeOrder.INSTANCE;
        ImmutableList<Node> featuresList = ImmutableList.of();
        ImmutableList<Node> treesList = ImmutableList.of();
    ...
    public static RevTreeImpl create(ObjectId id, long size, RevTree unidentified) {
        if (unidentified.buckets().isPresent()) {
            return new RevTreeImpl.NodeTree(id, size, unidentified.numTrees(),
                    unidentified.buckets().get());
        }
        ...
        return new RevTreeImpl.LeafTree(id, size, features, trees);
    }
