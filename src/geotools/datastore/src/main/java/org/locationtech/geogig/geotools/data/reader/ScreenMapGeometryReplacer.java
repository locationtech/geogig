 ...
 * screenmap) then the geometry is replaced with one that is a full pixel's size.
 * <p>
 * This is the same behaviour as the StreamingRenderer and ShapefileReader. If you don't do this the
 * resulting image looks "funny" due to anti-aliasing.
 * <p>
 * NOTE: its possible for the ScreenMapGeometryReplacer to be running at the same time as the
 * ScreenMapPredicate or FeatureScreenMapPredicate. However, the features handed to this class will
 * have already go through the filtering.
 */
public class ScreenMapGeometryReplacer implements Function<SimpleFeature, SimpleFeature> {
    ...
    /**
     * if the features is fully inside a pixel (screenMap.canSimplify()), then we replace it's
     * geometry with a one that takes up the entire pixels (screenMap.getSimplifiedShape()) OR with
     * the feature's boundingbox.
     * <p>
    ...
    public SimpleFeature apply(SimpleFeature feature) {
        ...
        if (!screenMap.canSimplify(e)) {
            ...
            if (replaceWithPixel) {
                Geometry newGeom = screenMap.getSimplifiedShape(e.getMinX(), e.getMinY(),
