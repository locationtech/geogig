 ...
 */
@RestController
@RequestMapping(path = "/" + BASE_REPOSITORY_ROUTE + "/{repoName}/repo/filteredchanges")
public class FilteredChangesController extends AbstractRepositoryController {

    private static final Logger LOGGER = LoggerFactory.getLogger(FilteredChangesController.class);
    ...
    public void postChanges(@PathVariable(name = "repoName") String repoName,
        ...
        if (optional.isPresent()) {
            ...
            FilteredChanges filterChanges = legacyFilteredChangesService.filterChanges(provider,
                    repoName, requestInput);
            // encode to Stream
            encodeToStream(filterChanges, request, response);
        } else {
            throw NO_PROVIDER;
        }
    }
