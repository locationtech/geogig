...
@Command(name = "rebuild", aliases = "rb", description = "Rebuild the index trees of an index for the whole history of the repository.")
public class RebuildIndex extends AbstractCommand implements CLICommand {

    @Option(names = "--tree", required = true, description = "Name or path of the feature tree to rebuild the index for.")
    private String treeRefSpec;

    @Option(names = { "-a", "--attribute" }, description = "Attribute to rebuild the index for.")
