...
/***
 * This speeds up filter evaluation by converting filters of the form; "property" = 'a' OR
 * "property" = 'b' OR "property" = 'c' TO IN("property", 'a','b','c') = true
 *
...
public class InReplacingFilterVisitor extends DuplicatingFilterVisitor {
    ...
    public List<Filter> simplify(List<Filter> filters, Object extraData) {
        ...
        for (Map.Entry<String, List<Object>> entry : grouped.entrySet()) {
            if (entry.getValue().size() == 1) {
                inMemmbers.add(createEquals(entry.getKey(), entry.getValue().get(0), extraData));
            } else {
                inMemmbers.add(createInFunction(entry.getKey(), entry.getValue(), extraData));
            }
    ...
    public PropertyIsEqualTo createEquals(String pname, Object literal, Object extraData) {
        ...
        return getFactory(extraData).equal(pnameFilter, literal1, true,
    ...
    public boolean isSimpleEquals(Filter expression) {
        if (expression instanceof PropertyIsEqualTo) {
            ...
            if ((equals.getExpression1() instanceof PropertyName)
                    && (equals.getExpression2() instanceof Literal) && (equals.isMatchingCase()))
                return true;
        }
        return false;
    }
