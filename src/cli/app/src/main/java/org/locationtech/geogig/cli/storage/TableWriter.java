...
public class TableWriter extends FilterWriter {
     ...
     * respectively.
     */
    private static final char[][] BOX = new char[][] { { // [0000]: single horizontal, single
                                                         // vertical
            '\u250C', '\u252C', '\u2510', '\u251C', '\u253C', '\u2524', '\u2514', '\u2534',
            '\u2518', '\u2500', '\u2502' },
    ...
    /**
     * Line position of the cell currently being written. The field is incremented each time
     * {@link #nextLine()} is invoked.
     */
    private int row;
    ...
    public TableWriter(final Writer out, final int spaces) {
        this(out, Strings.padStart("", spaces, ' '));
    }
     ...
     */
    private void writeBorder(final Writer out, final int horizontalBorder, final int verticalBorder,
            final char horizontalChar) throws IOException {
        /*
         * Obtient les ensembles de caractères qui conviennent pour la ligne horizontale.
         */
        int boxCount = 0;
        ...
        /*
         * Obtient une chaine contenant les lignes verticales à dessiner à gauche, à droite ou au
         * centre de la table.
         */
        final String border;
        ...
        /*
         * Remplace les espaces par la ligne horizontale, et les lignes verticales par une
         * intersection.
         */
     ...
     * tabulations (<code>'\t'</code>) characters are copied straight into the current cell, which
     * mean that next write operations will continue inside the same cell.
     * <li>If {@code false}, then tabulations move to next column and EOL move to the first cell of
     * next row (i.e. tabulation and EOL are equivalent to {@link #nextColumn()} and
     * {@link #nextLine()} calls respectively).
     * </ul>
     *
     ...
     * @param alignment Cell alignment. Must be {@link #ALIGN_LEFT} {@link #ALIGN_RIGHT} or
     *        {@link #ALIGN_CENTER}.
     */
     ...
     *
     * @return Cell alignment: {@link #ALIGN_LEFT} (the default), {@link #ALIGN_RIGHT} or
     *         {@link #ALIGN_CENTER}.
     */
    ...
    /**
     * Writes a portion of a string. Tabulations and line separators are interpreted as by
     * {@link #write(int)}.
     *
     * @param string String to write.
    ...
    /**
     * Writes an array of characters. Tabulations and line separators are interpreted as by
     * {@link #write(int)}.
     *
    ...
    public void nextColumn(final char fill) {
        synchronized (lock) {
            ...
            if (column >= width.length) {
                int[] resized = new int[column + 1];
                System.arraycopy(width, 0, resized, 0, width.length);
                width = resized;
            }
            int length = 0;
    ...
    public @Override void flush() throws IOException {
        synchronized (lock) {
            ...
            if (!(out instanceof TableWriter)) {
                /*
                 * Flush only if this table is not included in an outer (bigger) table. This is
                 * because flushing the outer table would break its formatting.
                 */
                out.flush();
            }
        }
    ...
    private void flushTo(final Writer out) throws IOException {
        ...
        for (int cellIndex = 0; cellIndex < cellCount; cellIndex++) {
            /*
             * Copie dans {@code currentLine} toutes les données qui seront à écrire sur la ligne
             * courante de la table. Ces données excluent le {@code null} terminal. La liste {@code
             * currentLine} ne contiendra donc initialement aucun élément nul, mais ses éléments
             * seront progressivement modifiés (et mis à {@code null}) pendant l'écriture de la
             * ligne dans la boucle qui suit.
             */
            Cell lineFill = null;
            int currentCount = 0;
            do {
            ...
            /*
             * La boucle suivante sera exécutée tant qu'il reste des lignes à écrire (c'est-à-dire
             * tant qu'au moins un élément de {@code currentLine} est non-nul). Si une cellule
             * contient un texte avec des caractères EOL, alors cette cellule devra s'écrire sur
             * plusieurs lignes dans la cellule courante.
             */
            while (!isEmpty(currentLine)) {
                for (int j = 0; j < currentLine.length; j++) {
                    ...
                    if (end >= 0) {
                        /*
                         * Si un retour chariot a été trouvé, n'écrit que la première ligne de la
                         * cellule. L'élément {@code currentLine[j]} sera modifié pour ne contenir
                         * que les lignes restantes qui seront écrites lors d'un prochain passage
                         * dans la boucle.
                         */
                        int top = end + 1;
                    ...
                    final int textLength = cellText.length();
                    /*
                     * Si la cellule à écrire est en fait une bordure, on fera un traitement spécial
                     * pour utiliser les caractres de jointures {@link #BOX}.
                     */
                    if (currentCount == 0) {
                        assert textLength == 0;
                    ...
                    /*
                     * Si la cellule n'est pas une bordure, il s'agit d'une cellule "normale".
                     * Procde maintenant à l'écriture d'une ligne de la cellule.
                     */
                    if (isFirstColumn) {
                        out.write(leftBorder);
                    }
                    final Writer tabExpander = (cellText.indexOf('\t') >= 0)
                            ? new ExpandedTabWriter(out)
                            : out;
