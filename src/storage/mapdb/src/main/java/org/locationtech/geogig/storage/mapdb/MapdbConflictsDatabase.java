...
class MapdbConflictsDatabase implements ConflictsDatabase {
	...
	//TODO: review immutability of Conflicts objects for MapdB Background Storage processes.
	private ConcurrentNavigableMap<Object[], Conflict> conflicts = null;
	...
	public List<Conflict> getConflicts(@Nullable String namespace,
		...
		if (pathFilter == null) {
			...
			*/
			...
			for (Map.Entry<Object[], Conflict> entry: conflicts.entrySet()) {
				Object[] key = entry.getKey();
				if (key.length > 0 && key[0] instanceof String && namespace.equals(key[0]))
				{
					matchingConflicts.add(entry.getValue());
				}
			}
		...
		return ImmutableList.copyOf(conflicts.subMap(fromKey, toKey).values());
	}
	...
	public void addConflict(@Nullable String namespace, Conflict conflict) {
		...
		conflicts.put(new Object[]{namespace,conflict.getPath()}, conflict);
	}
	...
	public Optional<Conflict> getConflict(@Nullable String namespace,
		...
		return Optional.fromNullable(conflicts.get(new Object[]{namespace, path}));
	}
