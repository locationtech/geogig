...
public class MapdbObjectDatabase implements ObjectDatabase {
	...
	public synchronized void open() {
		...
		//TODO will fail if /objects folder does not exist yet.
		db = DBMaker.fileDB(new File(repoLocation,"objects/objectdb.mapdb")).closeOnJvmShutdown()
				.make();

		// open existing an collection (or create new)
		collection = db.treeMap(getCollectionName());
	...
	public void configure() throws RepositoryConnectionException {
		RepositoryConnectionException.StorageType.OBJECT.configure(config,
				"mapdb", "0.1");
		// String uri =
		// config.get("mongodb.uri").or(config.getGlobal("mongodb.uri"))
		// .or("mongodb://localhost:27017/");
		// String database =
		// config.get("mongodb.database").or(config.getGlobal("mongodb.database"))
		// .or("geogig");
	...
	public synchronized void close() {
		...
		conflicts.close();
		conflicts = null;
		blobStore = null; //TODO do we need to close it?
	}
